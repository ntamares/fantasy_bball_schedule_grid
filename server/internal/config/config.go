...
func Load() *Config {
	...
	serverRoot := filepath.Join(filepath.Dir(filename), "../..")

	envPath := filepath.Join(serverRoot, ".env."+env)
	if err := godotenv.Load(envPath); err != nil {
		defaultEnvPath := filepath.Join(serverRoot, ".env")
		if err := godotenv.Load(defaultEnvPath); err != nil {
			log.Printf("Warning: Could not load .env file from %s: %v", defaultEnvPath, err)
		}
	}
