    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                throw new Error(`Failed to fetch weekly schedule: ${response.status} ${response.statusText} - ${errJson.message || text}`);
            } catch {
                throw new Error(`Failed to fetch weekly schedule: ${response.status} ${response.statusText} - ${text}`);
            }
        }
